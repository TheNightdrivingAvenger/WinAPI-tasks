...
OSASPLABAPI int WINAPI FindAndReplaceString(PINJECTEDPROCPARAMS params)
{
	DWORD findWhatLength = GetStringLength(params->findWhat);
	DWORD replaceWithLength = GetStringLength(params->replaceWith);
	...
	while (queryResult) {
		...
		if (queryResult && (mbi.State == MEM_COMMIT) && (currentAddress != params->findWhat)
			...
			for (DWORD i = 0; i < mbi.RegionSize - findWhatLength; i++) {
				if ((params->findWhat[stringPos] == ((char *)mbi.BaseAddress)[i]) && (stringPos < findWhatLength)) stringPos++;
				else stringPos = 0;
				
				if (stringPos == findWhatLength) {
					memcpy((char *)mbi.BaseAddress + i - findWhatLength + 1, params->replaceWith, findWhatLength);
					return 0;
				}
			}
		}
